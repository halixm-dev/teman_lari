...
void main() {
  group('TrainingLoadCalculator', () {
    ...
    test('should return empty list when no activities', () {
      final result = calculator.computeLoadHistory([], maxHr: 190, restingHr: 60);
      expect(result, isEmpty);
    });

    test('should calculate CTL, ATL, and TSB correctly using standard Coggan decay', () {
      // Let's create an activity on Monday, 3 days ago relative to now.
      final now = DateTime.now();
      final monday = DateTime(now.year, now.month, now.day).subtract(const Duration(days: 3));
      ...
      final activity = RunActivity(
        ...
        date: monday,
        distanceKm: 5.0,
        movingTime: const Duration(minutes: 34), // 34 minutes, no HR means TSS = 34 * 0.5 = 17
        pace: const Duration(minutes: 6, seconds: 48),
        elevationGainM: 10,
        trainingLoad: TrainingLoad.moderate,
      );

      final result = calculator.computeLoadHistory([activity], maxHr: 190, restingHr: 60);
